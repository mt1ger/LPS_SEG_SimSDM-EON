...
main() {
  ...
  static string Topo = "/home/mt1ger/"
                       "LPS_SEG_SimSDM-EON/Topology/US24.txt";
  static string Path = "/home/mt1ger/"
                       "LPS_SEG_SimSDM-EON/Plot/";
  ...
  for(unsigned int Max = 1; Max <= LTDOFSEG; Max *= 8) {
    ...
    for(unsigned int k = 0; k < Core.size(); k++) {
      ...
      string   titles;
      titles = "Core,Erlang,Seed,BP,maxTransponders,numTransponders_total,maxLPSs,"
      ...
      for(unsigned int seed = 0; seed < NUMOFSEEDS; seed++) {
        for(unsigned int lambda = 0; lambda < Lambda.size(); lambda++) {
          string Cmd = Exec + ' ' + Topo + ' ' + IsoFolder + ' '
                       ...
                       + to_string(MU) + ' ' + to_string(Max) + ' '
                       + to_string(MAXTIMESLICES) + ' ' + to_string(Seed[seed]);
  ...
  int cntSize = cmds[0].size();
  for(int j = 0; j < cmds[0].size(); j++) {
    int    threadsPerIteration = cmds.size();
    thread threads[threadsPerIteration];
    for(int i = 0; i < cmds.size(); i++) {
      cout << cmds[i][j] << endl;
      threads[i] = thread(thread_function, cmds[i][j]);
    }
