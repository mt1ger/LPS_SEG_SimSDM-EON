...
TrafficGenerator::gen_first_request()
{
  ...
  while(1)
  {
    ...
    if(requestTypeNum == 1 && network->numofARs != 0)
    {
      network->numofARs--;
      requestType   = c_AR;
    ...
    else if(requestTypeNum == 0 && network->numofIRs != 0)
    {
      network->numofIRs--;
      requestType = c_IR;
      break;
  ...
  // request = new CircuitRequest(src, dest, arrivalTime, bookAheadTime,
  // duration,
