...
Network::init()
{
  ...
  numRequest_400 = (long long)(numRequest * ratio_400);
  numRequest_100   = (long long)(numRequest * ratio_100);
  numRequest_40  = numRequest - numRequest_400 - numRequest_100;
  numRequest_AR = numRequest * AR_RATIO;
  numRequest_IR = numRequest - numRequest_AR;

  trafficGen_numRequest_AR = numRequest_AR;
  trafficGen_numRequest_IR  = numRequest_IR;
  trafficGen_numRequest_400 = numRequest_400;
  trafficGen_numRequest_100 = numRequest_100;
  trafficGen_numRequest_40  = numRequest_40;
...
Network::simulation()
{
  ...
  while(!eventQueue->ev_Queue.empty())
  {
    ...
    if(numSSs4Data > maxSSs4Data)
    {
      maxSSs4Data = numSSs4Data;
    }
    // End???
    ...
    if((totalBlocks + numAllocatedRequests) == numRequest)
      break;
  }
  ...
  // Begin??? Becuase of maxNumSSs4Data
  spectrumUtilization = (double)maxSSs4Data / NUMOFSPECTRALSLOTS;
  // End???

  avgHoldingTime      = totalHoldingTime / numAllocatedRequests;
  avgCores        = (double)totalCoresUsed / numAllocatedRequests;
  ...
  avgTransponders    = (double)totalTransponders / numAllocatedRequests;
  avgTransponders_AR = (double)totalTransponders_AR / totalAllocations_AR;
  avgTransponders_IR = (double)totalTransponders_IR / totalAllocations_IR;
  avgTransponders_400 = (double)totalTransponders_400 / totalAllocations_400;
  avgTransponders_400AR = (double)totalTransponders_400AR / totalAllocations_400AR;
  avgTransponders_400IR = (double)totalTransponders_400IR / totalAllocations_400IR;
  avgTransponders_100   = (double)totalTransponders_100 / totalAllocations_100;
  avgTransponders_100AR = (double)totalTransponders_100AR / totalAllocations_100AR;
  avgTransponders_100IR = (double)totalTransponders_100IR / totalAllocations_100IR;
  avgTransponders_40    = (double)totalTransponders_40 / totalAllocations_40;
  avgTransponders_40AR  = (double)totalTransponders_40AR / totalAllocations_40AR;
  avgTransponders_40IR  = (double)totalTransponders_40IR / totalAllocations_40IR;

  // LPSspR = (double)totalLPSs / numAllocatedRequests;
  // LPSspR_400 = (double)totalLPSs_400 / (numRequest * ratio_400);
  // LPSspR_400AR
  //     = (double)totalLPSs_400AR / (numRequest * ratio_400 * AR_RATIO);
  // LPSspR_400IR
  //     = (double)totalLPSs_400IR / (numRequest * ratio_400 * (1 -
  //     AR_RATIO));
  // LPSspR_100 = (double)totalLPSs_100 / (numRequest * ratio_100);
  // LPSspR_100AR
  //     = (double)totalLPSs_100AR / (numRequest * ratio_100 * AR_RATIO);
  // LPSspR_100IR
  //     = (double)totalLPSs_100IR / (numRequest * ratio_100 * (1 -
  //     AR_RATIO));
  // LPSspR_40  = (double)totalLPSs_40 / (numRequest * ratio_40);
  // LPSspR_40AR = (double)totalLPSs_40AR / (numRequest * ratio_40 *
  // AR_RATIO); LPSspR_40IR
  //     = (double)totalLPSs_40IR / (numRequest * ratio_40 * (1 - AR_RATIO));

  // cout << "Spectrum Utilization is: " << spectrumUtilization << endl;
  cout << "Number of Requests Breakdown: (AR, IR, 400, 100, 40)" << endl;
  cout << numRequest_AR << ' ' << numRequest_IR << ' '
       << numRequest_400 << ' ' << numRequest_100 << ' '
       << numRequest_40 << ' ' << endl;
  ...
  cout << "Average Utilization of Transponders Breakdown: (AR, IR, 400, 400AR, "
       ...
       << endl;
  cout << avgTransponders_AR << ' ' << avgTransponders_IR << ' ' << avgTransponders_400 << ' ' << avgTransponders_400AR << ' '
       << avgTransponders_400IR << ' ' << avgTransponders_100 << ' ' << avgTransponders_100AR << ' ' << avgTransponders_100IR
       << ' ' << avgTransponders_40 << ' ' << avgTransponders_40AR << ' ' << avgTransponders_40IR << endl;
  ...
  cout << "Blocking Probability: "
       << (double)totalBlocks / (double)numRequest << endl;
