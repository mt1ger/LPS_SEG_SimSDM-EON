...
Network::simulation()
{
  ...
  while(!eventQueue->ev_Queue.empty())
  {
    ...
    if(numofSections > maxNumofSections)
      maxNumofSections = numofSections;
    if(numofSS4Data > maxNumofSS4Data)
      maxNumofSS4Data = numofSS4Data;
  ...
  avgHybridFrag
      = (1 - (double)totalMDataSize / (totalSSOccupied * BW_SPECSLOT));
  ...
  cout << "Max # of Sections used for each request: " << maxNumofSections
       << endl;
