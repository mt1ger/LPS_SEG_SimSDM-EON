...
main()
{
  ...
  auto numofThreads = 4 * Core.size() * Seed.size() * Lambda.size();
  ...
  while(cnt < cntSize)
  {
    ...
    {
      cout << cmds[i][cnt] << endl;
      threads[threadsIter + i] = thread(thread_function, cmds[i][cnt]);
    }
    for(int i = 0; i < threadsPerIteration; i++)
    {
      threads[threadsIter + i].join();
    }
    cnt++;
